		...
		.extend({
			...
			ajaxFileUpload : function(k) {
				...
				var d = function(l) {
					...
					try {
						if (p.contentWindow) {
							f.responseText = p.contentWindow.document.body ? p.contentWindow.document.body.innerHTML
									: null;
							f.responseXML = p.contentWindow.document.XMLDocument ? p.contentWindow.document.XMLDocument
									: p.contentWindow.document
						} else {
							if (p.contentDocument) {
								f.responseText = p.contentDocument.document.body ? p.contentDocument.document.body.innerHTML
										: null;
								f.responseXML = p.contentDocument.document.XMLDocument ? p.contentDocument.document.XMLDocument
				...
				if (window.attachEvent) {
					...
					document.getElementById(h).addEventListener("load", d,
							false)
				}
				return {
					abort : function() {
...
var G_MAPS_KEY = "ABQIAAAAXTuVPZjH3Urm8Qv_ZXDcEhQlH4KwB_SkVTDcGCQPbCQOgVLxUxQv225PONsf6ObCH3NJosi2T2Z4YQ";
var __bootstrap = {
	init : function() {
		__bootstrap.loader = new YAHOO.util.YUILoader({
			require : [ "container" ],
			loadOptional : true,
			onSuccess : function() {
				__ui.init();
				window.alert = function(a) {
					__ui.alertInfo.setBody(a);
					__ui.alertInfo.show()
				}
			}
		});
		__bootstrap.loader.insert()
	}
};
var __ui = {
	init : function() {
		__ui.alertInfo = new YAHOO.widget.SimpleDialog("__simplealert", {
			width : "300px",
			fixedcenter : true,
			visible : false,
			draggable : true,
		...
		__ui.alertInfo.setHeader("Información");
		__ui.alertInfo.render(document.body)
	}
};
__bootstrap.init();
$.download = function(b, c, d) {
	if (b && c) {
		...
		$.each(c.split("&"), function() {
			var e = this.split("=");
			a += '<input type="hidden" name="' + e[0] + '" value="' + e[1]
					+ '" />'
		});
		$(
				'<form action="' + b + '" method="' + (d || "post") + '">' + a
...
Date.CultureInfo = {
	...
	monthNames : [ "enero", "febrero", "marzo", "abril", "mayo", "junio",
			...
			"diciembre" ],
	abbreviatedMonthNames : [ "ene", "feb", "mar", "abr", "may", "jun", "jul",
			"ago", "sep", "oct", "nov", "dic" ],
	amDesignator : "",
	pmDesignator : "",
	firstDayOfWeek : 1,
...
Date.getMonthNumberFromName = function(b) {
	var e = Date.CultureInfo.monthNames, a = Date.CultureInfo.abbreviatedMonthNames, d = b
			.toLowerCase();
...
Date.getDayNumberFromName = function(b) {
	var f = Date.CultureInfo.dayNames, a = Date.CultureInfo.abbreviatedDayNames, e = Date.CultureInfo.shortestDayNames, d = b
			.toLowerCase();
...
Date.getDaysInMonth = function(a, b) {
	return [ 31, (Date.isLeapYear(a) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30,
			31, 30, 31 ][b]
};
Date.getTimezoneOffset = function(a, b) {
	return (b || false) ? Date.CultureInfo.abbreviatedTimeZoneDST[a
			.toUpperCase()] : Date.CultureInfo.abbreviatedTimeZoneStandard[a
			.toUpperCase()]
};
Date.getTimezoneAbbreviation = function(b, d) {
	var c = (d || false) ? Date.CultureInfo.abbreviatedTimeZoneDST
			: Date.CultureInfo.abbreviatedTimeZoneStandard, a;
	for (a in c) {
		if (c[a] === b) {
			return a
		}
...
Date.prototype.set = function(b) {
	...
	if (a.day != -1
			&& Date.validateDay(a.day, this.getFullYear(), this.getMonth())) {
...
Date.prototype.getDayName = function(a) {
	return a ? Date.CultureInfo.abbreviatedDayNames[this.getDay()]
			: Date.CultureInfo.dayNames[this.getDay()]
};
Date.prototype.getMonthName = function(a) {
	return a ? Date.CultureInfo.abbreviatedMonthNames[this.getMonth()]
...
Date.prototype.toString = function(c) {
	...
	return c ? c.replace(/dd?d?d?|MM?M?M?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|zz?z?/g,
			function(d) {
				switch (d) {
				case "hh":
					return b(a.getHours() < 13 ? a.getHours()
							: (a.getHours() - 12));
				case "h":
					return a.getHours() < 13 ? a.getHours()
							: (a.getHours() - 12);
				case "HH":
					return b(a.getHours());
				case "H":
				...
				case "M":
					return a.getMonth() + 1;
				case "t":
					return a.getHours() < 12 ? Date.CultureInfo.amDesignator
...
(function() {
	...
	var p = ("sunday monday tuesday wednesday thursday friday saturday")
			.split(/\s/), o = ("january february march april may june july august september october november december")
			.split(/\s/), n = ("Millisecond Second Minute Hour Day Week Month Year")
			.split(/\s/), m;
	var l = function(i) {
		return function() {
			if (this._is) {
...
(function() {
	...
	var c = a.Operators = {
		...
		list : function(i, j, k) {
			j = j || c.rtoken(/^\s*/);
			k = k || null;
			return (i instanceof Array ? c.each(c.product(i.slice(0, -1), c
					.ignore(j)), i.slice(-1), c.ignore(k)) : c.each(c.many(c
					.each(i, c.ignore(j))), px, c.ignore(k)))
		},
		set : function(i, j, k) {
			j = j || c.rtoken(/^\s*/);
			k = k || null;
...
(function() {
	...
	Date.Translator = {
		...
		month : function(g) {
			return function() {
				this.month = ((g.length == 3) ? Date.getMonthNumberFromName(g)
						: (Number(g) - 1))
			}
		},
		year : function(g) {
			return function() {
				var j = Number(g);
				this.year = ((g.length > 2) ? j
						: (j + (((j + 2000) < Date.CultureInfo.twoDigitYearMax) ? 2000
								: 1900)))
			}
		},
		rday : function(g) {
		...
		finishExact : function(g) {
			...
			var l = new Date(this.year, this.month, this.day, this.hour,
		...
		finish : function(g) {
			...
			if (n) {
				var o, l, k;
				k = ((this.orient == "past" || this.operator == "subtract") ? -1
						: 1);
				if (this.weekday) {
					this.unit = "day";
				...
				if (this.weekday && !this.day) {
					this.day = (j.addDays((Date
							.getDayNumberFromName(this.weekday) - j.getDay())))
							.getDate()
				}
	...
	e.d = b.cache(b.process(b.each(b.rtoken(/^([0-2]\d|3[0-1]|\d)/), b
			.optional(e.ctoken2("ordinalSuffix"))), d.day));
	e.dd = b.cache(b.process(b.each(b.rtoken(/^([0-2]\d|3[0-1])/), b.optional(e
			.ctoken2("ordinalSuffix"))), d.day));
	e.ddd = e.dddd = b.cache(b.process(e.ctoken("sun mon tue wed thu fri sat"),
			function(g) {
				return function() {
					this.weekday = g
				}
	...
	i = function() {
		return b.each(b.any.apply(null, arguments), b.not(e
				.ctoken2("timeContext")))
	};
	e.day = i(e.d, e.dd);
	e.month = i(e.M, e.MMM);
	...
	e.unit = b.process(e.ctoken("minute hour day week month year"),
			function(g) {
				return function() {
					this.unit = g
				}
	...
	e.expression = b.set([ e.rday, e.operator, e.value, e.unit, e.orientation,
			e.ddd, e.MMM ]);
	i = function() {
	...
	e.format = b.process(b.many(b.any(b.process(b
	...
	e._start = b.process(b.set([ e.date, e.time, e.expression ],
			e.generalDelimiter, e.whiteSpace), d.finish);
	e.start = function(g) {
		try {
...
var BASE = "/gestplus";
YAHOO.namespace("gestplus");
var DEFAULT_ROWS_PER_PAGE = 10;
var _get_query_string = function(c) {
	var b = c ? c : {};
	$.each($("div#filters .f").serializeArray(), function(d, e) {
		b[e.name] = e.value
	});
	var a = $("input#checkbox_old").attr("checked");
	b.filterOld = a;
	return _tx_params(b)
};
var _tx_params = function(c) {
	var b = [];
	for (name in c) {
		b.push(name + "=" + c[name])
	}
...
var refresh_data = function(b, c) {
	showCustomLoading();
	var d = {
		success : function(f, e, g) {
			hideCustomLoading();
			var h = this.get("paginator");
			this.onDataReturnReplaceRows(f, e, g);
			h.set("totalRecords", e.meta.totalRecords)
		},
		failure : function(f, e, g) {
			hideCustomLoading();
			alert("No ha sido posible actualizar los datos de la tabla.")
		},
		scope : b
	};
	var a = myQueryStringBuilder(b.getState());
	b.getDataSource().sendRequest("filter=true" + a, d)
};
var myQueryStringBuilder = function(c, b) {
	var d = {
		startIndex : c.pagination.recordOffset,
		results : c.pagination.rowsPerPage
	};
	if (c.sortedBy) {
		d.sort = c.sortedBy.key;
		d.dir = (c.sortedBy.dir === YAHOO.widget.DataTable.CLASS_DESC) ? "desc"
				: "asc"
	}
	var a = _get_query_string(d);
	return a
};
var download_data = function(e, f) {
	var a = myQueryStringBuilder(e.getState());
	var c = _tx_params(f);
	var b = BASE + "/incidencia/filter?download=true" + a + c;
	var d = $("<form id='__form' method='POST' action=\"" + b + '" />').css({
		display : "none"
	});
	$("body").append(d);
	d.submit().remove();
	alert("Se est� generando el informe seleccionado. La descarga del fichero comenzar� en unos instantes. Por favor, espere.")
};
var handle_data_payload = function(b, a, c) {
	if (c) {
		c.totalRecords = a.meta.totalRecords
	}
	return c
};
var delete_row_formatter = function(b, a, c, d) {
	b.innerHTML = '<img src="'
			+ BASE
			+ '/resources/image/icons/16x16/actions/list-remove.png" title="borrar" alt="borrar" />';
	...
	showCustomLoading();
			$.ajax({
				...
				error : function(c, e, d) {
					hideCustomLoading();
					alert("No se ha podido cargar el elemento. Reintente la operación pasados unos instantes y, si el problema persiste, contacte con el administrador del sistema. Disculpe las molestias.")
				},
				dataType : "html"
			})
};
var load_historicoAACC = function(a, b) {
	showCustomLoading();
			$.ajax({
				url : a,
				type : "POST",
...
var load_ejercicio = function(a, b) {
	showCustomLoading();
			$.ajax({
				url : a,
				type : "POST",
...
var load_item_data = function(a, b) {
	showCustomLoading();
			$.ajax({
				url : a,
				type : "POST",
...
var handle_errors = function(d) {
	var a = false;
	if (d && d.messages) {
		a = true;
		if (d.messages.fielderrors) {
			alert("Alguno de los datos introducidos no es válido");
			for (key in d.messages.fielderrors) {
				var f = $("span#_ph_" + key);
				if (f) {
					var c = d.messages.fielderrors[key];
					var e = "";
